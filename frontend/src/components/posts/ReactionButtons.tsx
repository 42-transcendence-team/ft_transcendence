...
import { dislikePost, likePost, undislikePost, unlikePost } from 'api/Posts';
...
export const ReactionButtons = ({
  ...
  const [loadingAction, setLoadingAction] = useState<ReactionAction | null>(
    null,
  );
      ...
      >
        <button
          className={`reaction-buttons__control reaction-buttons__control--like${
            likedByCurrentUser ? ' reaction-buttons__control--active' : ''
          }`}
          type="button"
          ...
          aria-label={
            likedByCurrentUser
              ? 'Retirar Me gusta'
              : 'Indicar que te gusta la publicación'
          }
        >
          <span className="reaction-buttons__icon" aria-hidden="true">
            {likedByCurrentUser ? <FaThumbsUp /> : <FaRegThumbsUp />}
          </span>

          <span className="reaction-buttons__count">{likeCount}</span>
        </button>

        <button
          className={`reaction-buttons__control reaction-buttons__control--dislike${
            dislikedByCurrentUser ? ' reaction-buttons__control--active' : ''
          }`}
          type="button"
        ...
        >
          <span className="reaction-buttons__icon" aria-hidden="true">
            {dislikedByCurrentUser ? <FaThumbsDown /> : <FaRegThumbsDown />}
          </span>

          <span className="reaction-buttons__count">{dislikeCount}</span>
        </button>
      </div>

      {error && <p className="reaction-buttons__error">{error}</p>}
    </div>
  );
};
