...
type PostCardProps = {
  ...
  onReactionUpdated: (reactionState: PostReactionState) => void;
};
...
function getAttachmentName(post: PostSummary): string {
...
function getAbsolutePostDate(value: string): string {
  const timestamp = Date.parse(value);
...
function hasImageVariant(variant: PostCardVariant): boolean {
  return variant === 'image' || variant === 'text-image';
}

function hasPdfVariant(variant: PostCardVariant): boolean {
  return variant === 'pdf' || variant === 'text-pdf';
}
...
export const PostCard = ({
  ...
  const profilePath = `/app/profile/${encodeURIComponent(post.author.login)}`;

  const imageSrc =
    hasImageVariant(variant) && post.imagePath
  ...
  return (
    <article className={['post-card', `post-card--${variant}`].join(' ')}>
      {/*
      ...
      <button
        ...
        type="button"
        aria-label={`Abrir la publicación de ${post.author.login}`}
      ...
      <div className="post-card__body">
        <header className="post-card__header">
          <div className="post-card__author">
            ...
            <Link className="post-card__username" to={profilePath}>
              {post.author.login}
            </Link>
          </div>
          ...
          <time
            ...
            title={getAbsolutePostDate(post.createdAt)}
          >
        ...
        {imageSrc && (
          <div className="post-card__media">
            <img
              ...
              src={imageSrc}
              alt={`Publicación de ${post.author.login}`}
              loading="lazy"
            />
          </div>
        )}
        ...
        {hasPdfVariant(variant) && (
          <div className="post-card__document">
            ...
            <div className="post-card__document-info">
              <span className="post-card__document-name">{attachmentName}</span>

              <span className="post-card__document-type">PDF</span>
            </div>
          </div>
        )}
