...
export const MobileBottomNav = () => {
  ...
  useEffect(() => {
    if (!authenticatedUser?.login) return;
    let cancelled = false;
    ...
    getUserProfile(authenticatedUser.login, { noIncrement: true })
      ...
      .catch((error) => {
        console.error("Error cargando perfil en MobileBottomNav", error);
      });
  ...
  return (
    <nav className="mobileBottomNav">
      ...
      {/* 2. Búsqueda */}
      <NavLink 
        to="/XXXXXXX" //ns q enlace va aqui
        className={
          ({ isActive }) => isActive 
          ? "mobileBottomNav__link active" : "mobileBottomNav__link"
        }
      ...
      <NavLink 
        to="/app/posts/new" // aqui iria la peticion para subir un post 
        className={
          ({ isActive }) => isActive 
          ? "mobileBottomNav__link active" : "mobileBottomNav__link"
        }
      ...
      {/* 4. Mensajes / Chat */}
      <NavLink 
        to="/chat" // para los chat ns q url es
        className={
          ({ isActive }) => isActive 
          ? "mobileBottomNav__link active" : "mobileBottomNav__link"
        }
      ...
      <NavLink 
        to={authenticatedUser?.login ? `/app/profile/${authenticatedUser.login}` : "/app"} 
        className={
          ({ isActive }) => isActive 
          ? "mobileBottomNav__link active" : "mobileBottomNav__link"
        }
