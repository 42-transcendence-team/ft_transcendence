...
export function ChatModal({ id, onClose }: ChatModalProps) {
  const { messagesByRoom, sendMessage, user, roomMembers, joinRoom } =
    useChat();
  const navigate = useNavigate();
  const messagesRef = useRef<HTMLDivElement>(null);
  const modalRef = useRef<HTMLDivElement>(null);
  const otherMember = (roomMembers[id] || []).find(
    (m) => m.id !== parseInt(user?.id || '0', 10),
  );
  ...
  useEffect(() => {
    const handleClickOutside = (e: PointerEvent) => {
      ...
      if (target instanceof Element) {
        ...
        if (
          target.closest(
            '.chatPanel__bubble, .notification-item, .mobileChatSheet, .chatPanel__actions, .addChatModal__backdrop, .mobileChatSheet__backdrop',
          )
        ) {
          return;
        }
    ...
    <div className="chatModal" ref={modalRef}>
      <div className="chatModal__header">
        {otherMember ? (
          ...
          >
            <UserAvatar
              avatarPath={otherMember.avatar_url || null}
              username={otherMember.login}
              size="small"
            />
      ...
      <form autoComplete="off" action={handleSend} className="chatModal__form">
        <input
          ...
          maxLength="512"
        />
        <button type="submit" className="chatModal__form--btn">
          Enviar
        </button>
      </form>
    </div>
