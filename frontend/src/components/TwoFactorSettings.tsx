...
import { Footer2FA, OtpInput, TwoFactorQRCode } from '@components/TwoFactorUI';
...
const handleToggle = async (checked: boolean) => {
  ...
  const handleClose = () => {
    if (animating === 'on') setActive2FA(false);
    setAnimating(null);
    setModalOpen(false);
    setOtpCode(Array(CODE_LENGTH).fill(''));
  };
  ...
  return (
    <>
      ...
      >
        ...
        {mode === 'enable' && qr && <TwoFactorQRCode qrBase64={qr} />}

        <OtpInput onChange={(code) => setOtpCode(code.slice(0, CODE_LENGTH))} />
