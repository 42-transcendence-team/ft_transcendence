...
import { deleteBanner, updateBanner } from '../../api/UserBanner';
...
function getBannerErrorCode(error: unknown): string | null {
...
function getBannerSource(bannerPath: string): string {
  return bannerPath.startsWith('/') ? bannerPath : `/${bannerPath}`;
}
...
export function BannerEditorModal({
  ...
  const showCurrentBanner =
    !file && Boolean(currentBannerPath) && !currentPreviewFailed;
    ...
    >
      {!file && (
        <div className="banner-editor-modal__current-preview">
          {showCurrentBanner && currentBannerPath ? (
            <img
              className="banner-editor-modal__banner"
              src={getBannerSource(currentBannerPath)}
              alt="Imagen de cabecera actual"
              onError={() => setCurrentPreviewFailed(true)}
            />
          ) : (
            <div className="banner-editor-modal__placeholder">
              <i className="fas fa-image" />
              <span>No hay ninguna imagen de cabecera seleccionada</span>
            </div>
          )}
        </div>
      )}
      ...
      {validationError && (
        <p className="banner-editor-modal__error">{validationError}</p>
      )}

      {requestError && (
        <p className="banner-editor-modal__error">{requestError}</p>
      )}
      ...
      <div className="modal__footer banner-editor-modal__actions">
        ...
        <button
          ...
          onClick={handleSave}
        >
          {operation === 'save' ? 'Guardando...' : 'Guardar imagen de cabecera'}
        </button>
      </div>
    </Modal>
  );
