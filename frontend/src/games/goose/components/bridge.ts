...
export async function animateBridge(
  action: GooseAction,
...
export function drawBridgeAnimation(
  ...
  for (let i = 0; i < plankCount; i++) {
    const plankProgress = Math.min(
      Math.max(bridgeProgress * plankCount - i, 0),
      1,
    );
    ...
    const plankX = bridgeLeft + i * (bridgeWidth / plankCount) + gap / 2;
    ...
    ctx.translate(plankX + plankWidth / 2, bridgeTop + bridgeHeight / 2 - drop);
  ...
  if (bridgeProgress > 0.3) {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      ctx.lineTo(supportX, bridgeTop + bridgeHeight * 0.5);
      ctx.stroke();
    }
  }
