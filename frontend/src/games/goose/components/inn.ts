...
export async function animateInn(
  action: GooseAction,
  ...
  context.state.message =
    action.payload ?? 'Te quedas en la posada y pierdes un turno';
...
export function drawInnAnimation(
  ...
  const radius =
    minDimension * 0.08 + (maxRadius - minDimension * 0.08) * sceneProgress;
  ...
  const stars = [
    ...
    { x: 0.4, y: 0.14, size: 0.014 },
    { x: 0.58, y: 0.2, size: 0.018 },
    { x: 0.76, y: 0.14, size: 0.013 },
    { x: 0.88, y: 0.3, size: 0.018 },
    { x: 0.16, y: 0.7, size: 0.013 },
    { x: 0.3, y: 0.82, size: 0.018 },
    { x: 0.7, y: 0.76, size: 0.014 },
  ...
  ctx.beginPath();
  ctx.roundRect(
    centerX - bedWidth * 0.3,
    mattressY - bedHeight * 0.12,
    bedWidth * 0.62,
    bedHeight * 0.78,
    bedHeight * 0.2,
  );
  ...
  ctx.beginPath();
  ctx.roundRect(
    centerX - bedWidth * 0.42,
    mattressY - bedHeight * 0.16,
    bedWidth * 0.24,
    bedHeight * 0.68,
    bedHeight * 0.2,
  );
  ...
  const personX = centerX + bedWidth * 0.1;
  const personY = mattressY + bedHeight * 0.2;
  ...
  ctx.beginPath();
  ctx.roundRect(
    personX - bedWidth * 0.16,
    personY + personRadius * 0.35,
    bedWidth * 0.3,
    bedHeight * 0.4,
    bedHeight * 0.18,
  );
  ...
  const cloudAlpha = alpha * (0.15 + sceneProgress * 0.2);
  ...
  for (const offset of [-1, 0, 1]) {
    ctx.beginPath();
    ctx.arc(
      centerX + offset * cloudSize * 1.2,
      cloudY,
      cloudSize * (0.7 + Math.abs(offset) * 0.2),
      0,
      Math.PI * 2,
    );
    ctx.fill();
