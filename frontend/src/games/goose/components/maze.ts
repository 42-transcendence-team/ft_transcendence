...
export async function animateMaze(
  action: GooseAction,
...
export function drawMazeAnimation(
  ...
  ctx.lineWidth = Math.max(2, minDimension * 0.012);
  ...
  for (let i = 0; i < rings; i++) {
    ...
    const ringAlpha = Math.min(
      1,
      Math.max(0, (mazeProgress - delay) / (1 - delay)),
    );
  ...
  for (let i = 1; i < 5; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(centerX - mazeSize * 0.42, centerY + offset);

    ctx.lineTo(
      centerX + mazeSize * (i % 2 === 0 ? 0.18 : 0.42),
      centerY + offset,
    );
    ...
    ctx.lineTo(
      centerX + offset,
      centerY + mazeSize * (i % 2 === 0 ? 0.18 : 0.42),
    );
    ctx.stroke();
  }
  ...
  if (progress > 0.72) {
    ...
    ctx.arc(
      centerX,
      centerY,
      closeProgress * minDimension * 0.3,
      0,
      Math.PI * 2,
    );
    ctx.fill();
