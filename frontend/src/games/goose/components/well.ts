...
export async function animateWell(
  action: GooseAction,
  ...
  context.state.message =
    action.payload ??
    'Caes en el pozo y pierdes turnos hasta que otro jugador caiga aquí';
...
export function drawWellAnimation(
  ...
  const maxRadius = Math.sqrt(
    Math.max(x, canvasWidth - x) ** 2 + Math.max(y, canvasHeight - y) ** 2,
  );
