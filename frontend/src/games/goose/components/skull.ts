...
import { animateSpecialEffect, wait } from './actions';

export async function animateSkull(
  action: GooseAction,
...
export function drawSkullAnimation(
  ...
  for (let crackIndex = 0; crackIndex < cracks.length; crackIndex++) {
    ...
    const localProgress = Math.max(
      0,
      Math.min(1, (crackProgress - delay) / (1 - delay)),
    );
  ...
  const skullProgress = Math.min(Math.max((progress - 0.1) / 0.45, 0), 1);
  ...
  const shake =
    progress > 0.25 && progress < 0.75
      ? Math.sin(progress * Math.PI * 24) * minDimension * 0.006
      : 0;
  ...
  ctx.quadraticCurveTo(
    skullX - skullWidth * 0.46,
    skullY - skullHeight * 0.5,
    skullX,
    skullY - skullHeight * 0.52,
  );

  ctx.quadraticCurveTo(
    skullX + skullWidth * 0.46,
    skullY - skullHeight * 0.5,
    skullX + skullWidth * 0.43,
    skullY - skullHeight * 0.18,
  );

  ctx.lineTo(skullX + skullWidth * 0.34, skullY + skullHeight * 0.22);
  ctx.lineTo(skullX + skullWidth * 0.2, skullY + skullHeight * 0.38);
  ctx.lineTo(skullX - skullWidth * 0.2, skullY + skullHeight * 0.38);
  ctx.lineTo(skullX - skullWidth * 0.34, skullY + skullHeight * 0.22);
  ...
  for (let i = 0; i < teeth; i++) {
    ...
    const toothHeight = skullHeight * 0.1;
    const toothX = skullX - skullWidth * 0.14 + i * skullWidth * 0.055;

    ctx.fillRect(toothX, skullY + skullHeight * 0.2, toothWidth, toothHeight);
  }

  if (progress > 0.7) {
    const flashProgress = (progress - 0.7) / 0.3;

    ctx.globalAlpha = alpha * flashProgress * 0.2;
    ...
    ctx.arc(
      centerX,
      centerY,
      minDimension * (0.12 + flashProgress * 0.2),
      0,
      Math.PI * 2,
    );
