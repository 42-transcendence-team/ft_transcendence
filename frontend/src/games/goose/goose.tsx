...
import { useGoose, type GooseAction, type GoosePlayerState } from "games/goose/useGoose";
import { drawBoard, drawPlayers } from "./components/board";
import { drawDice, drawDiceAnimation } from "./components/dice";
import { getPosition, playActions, type GooseAnimationState } from "./components/actions";
...
export function drawMessage(ctx: CanvasRenderingContext2D,
	message: string, canvasWidth: number, canvasHeight: number) {
	ctx.save();
...
function drawSpecialAnimation(ctx: CanvasRenderingContext2D,
	animation: NonNullable<GooseAnimationState["specialAnimation"]>,
	players: Record<string, GoosePlayerState>, cellSize: number) {
	...
	switch (animation.type) {
		...
		case "prison":
			drawPrisonAnimation(ctx, animation.progress, ctx.canvas.width, ctx.canvas.height);
			break;
		case "finish":
			drawFinishAnimation(ctx, animation.progress, players, animation.token, cellSize);
			break;
	}
}
...
export function Goose() {
	...
	if (animationState?.gooseAnimation != null) {
		drawFlyingGeese(ctx, animationState.gooseAnimation, canvas.width, canvas.height);
	}

	if (animationState?.specialAnimation) {
		drawSpecialAnimation(ctx, animationState.specialAnimation, players, cellSize);
	}
	...
	useEffect(() => {
		...
		for (const action of actions) {
			if (action.type !== "move" ||action.from === undefined ||action.token === undefined) {
				continue;
			}
