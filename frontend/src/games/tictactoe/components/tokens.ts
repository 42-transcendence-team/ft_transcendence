...
function drawPencilX( ctx: CanvasRenderingContext2D, x: number, y: number, size: number) {
...
function drawPencilO(
	...
	for (let i = 0; i <= points; i++) {
		...
		const variation = 1 + Math.sin(angle * 3.0) * 0.025 + Math.sin(angle * 5.0) * 0.015;
...
export function drawPiece({ ctx, x, y, size, type, preview = false }: DrawPieceOptions) {
	ctx.save();
