...
export function ConnectFour() {
	...
	const draw = useCallback(() => {
		...
		backendBoard.forEach((row, rowIndex) => {
			row.forEach((value, columnIndex) => {
				...
				drawPiece({ctx, x, y, width: cellWidth, height: cellHeight, type: value});
			});
		});
		...
		if (gameState.status === "PLAY" && mouseRef.current.x >= 0) {
			...
			if (column >= 0 && column < CONNECT_FOUR_COLUMNS) {
				...
				if (row >= 0) {
					...
					drawPiece({ctx, x, y, width: cellWidth, height: cellHeight, 
						type: getPieceFromTurn( gameState.turn ), preview: true });
				}
			}
		}
	...
	return (
		<canvas ref={canvasRef} width={1050} height={900}
			onClick={handleClick}
