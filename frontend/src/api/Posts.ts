...
export async function getFeedPosts(page = 1, limit = 20): Promise<PostsPage> {
  return apiRequest<PostsPage>({
    endpoint: buildPostListEndpoint('posts/feed', page, limit),
    method: 'GET',
  });
}
...
export async function getPostsByUserId(
  ...
  return apiRequest<PostsPage>({
    endpoint: buildPostListEndpoint(`posts/user/${userId}`, page, limit),
    method: 'GET',
  });
}
