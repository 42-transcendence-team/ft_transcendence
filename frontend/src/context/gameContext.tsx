...
export function GameProvider({
  ...
  useEffect(() => {
    ...
    const unsubscribeGameUpdate = subscribe('game_update', (message: any) => {
      ...
      setGameState((prevState) => ({
        ...
        last_dice_roll:
          message.state?.last_dice_roll ?? prevState.last_dice_roll,
    ...
    const unsubscribeGameFinished = subscribe(
      'game_finished',
      (message: any) => {
        setGameState((prevState) => ({
          ...prevState,
    ...
    const unsuscribePlayerDisconnected = subscribe(
      'player_disconnected',
      (message: any) => {
    ...
    const unsubscribePlayerTimeout = subscribe(
      'player_timeout',
      (message: any) => {
    ...
    const unsubscribeGameJoined = subscribe(
      'game_join_error',
      (message: any) => {
        setGameState((prevState) => ({
          ...prevState,
    ...
    const unsubscribeGameJoinedSuccess = subscribe(
      'game_joined',
      (message: any) => {
        if (!message.state?.id) {
  ...
  const createGame = useCallback(
    (gameType: GameState['game_type'], mode: GameMode, players: number) => {
      if (!user) return;
      console.log(
        `Creating game of type: ${gameType} with mode: ${mode} and players: ${players}`,
      );
      send({
        type: 'game',
        payload: {
