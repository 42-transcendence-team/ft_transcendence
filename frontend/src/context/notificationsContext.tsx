...
const normalizeNotifications = (data: any[]): Notification[] => {
  return data.map((n: any, i: number) => ({
    ...
    id:
      n.id ??
      `${n.type}_${n.payload?.id ?? n.payload?.room_id ?? n.payload?.post_id ?? i}`,
  }));
};

const getNotifications = async (
  updateNotifications: (data: Notification[]) => void,
) => {
  try {
...
export const useHandleNotification = (
  ...
  useEffect(() => {
    ...
    setNotifications((prev) => {
      ...
      return currentNotis.filter(
        (n) =>
          !(n.type === 'UNREAD_MESSAGES' && n.payload?.room_id === activeChat),
      );
    });
  }, [activeChat]);
  ...
  useEffect(() => {
      ...
      setNotifications((prevNotis) => {
        ...
        const exists = currentNotis.some(
          (n) =>
            ...
            senderIdOf(n.payload) === senderIdOf(message.payload),
        );
        if (exists) return currentNotis;
        return [
          {
            id:
              message.id ||
              `friend_req_${senderIdOf(message.payload) ?? Date.now()}`,
            type: 'FRIEND_REQUEST',
      ...
      setNotifications((prevNotis) => {
        ...
        const filtered = currentNotis.filter(
          (n) =>
            !(
              n.type === 'FRIEND_REQUEST' &&
              n.payload?.sender_id === originalSenderId
            ),
        );
          ...
          return [
            {
              id:
                message.id ||
                `friend_accepted_${acceptedByUserId}_${Date.now()}`,
    ...
    const handleGenericNotification = (message: any) => {
      ...
      if (!type || !genericTypes.includes(type)) return;
      ...
      setNotifications((prevNotis) => {
        ...
        return [
          {
            id:
              message.id || `${type}_${message.payload?.post_id}_${Date.now()}`,
            type,
            payload: message.payload,
            createdAt: new Date().toISOString(),
          },
    ...
    const unsubFriendAccepted = subscribe(
      'FRIEND_REQUEST_ACCEPTED',
  ...
  const markAsRead = useCallback((notificationId: string | number) => {
    ...
    if (typeof notificationId === 'number') markAsReadApi(notificationId);
  }, []);
