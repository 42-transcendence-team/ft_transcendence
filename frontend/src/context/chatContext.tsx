...
export function ChatProvider({
  ...
  const [sendRejection, setSendRejection] = useState<SendRejection | null>(
    null,
  );
  useJoinRooms(rooms);
    ...
    const unsubscribeMessage = subscribe('message', (message: any) => {
      ...
      if (message.content) {
        ...
        setMessagesByRoom((prev) => {
          ...
          if (msgs.some((m) => m.message_id === message_id)) return prev;
      ...
      (message: any) => {
        ...
        setMessagesByRoom((prev) => {
          ...
          const cleaned = msgs.filter(
            (m) => !(m as any)._pending || m.content !== content,
          );
        ...
        setSendRejection({
          roomId: room_id,
          reason: reason ?? 'No puedes enviar este mensaje',
        });
      },
    );
