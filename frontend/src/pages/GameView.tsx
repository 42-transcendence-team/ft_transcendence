...
function GameViewLobby() {
  ...
  return (
    <div className="game-menu">
      ...
      <h3 className="game-menu__subtitle">
        Código de la partida: {gameState.game_id}
      </h3>
...
function GameViewJoin() {
  ...
  const [code, setCode] = useState(
    gameState.game_id ? String(gameState.game_id) : '',
  );
  ...
  return (
    <div className="game-menu">
      ...
      />
      {gameState.error && (
        <div className="game-menu__error">{gameState.error}</div>
      )}
      <button
...
function getWinnerName() {
  ...
  if (gameState.mode === 'local') {
    return (
      LOCAL_PLAYER_NAMES[gameState.game_type]?.[gameState.winner] ||
      'Jugador desconocido'
    );
  }
...
function getPlayerName() {
  ...
  const player = gameState.players.find((p) => p.token === gameState.turn);
  if (!player) return 'Jugador desconocido';

  if (gameState.mode === 'local') {
    return (
      LOCAL_PLAYER_NAMES[gameState.game_type]?.[player.token] ||
      'Jugador desconocido'
    );
  }
...
function GameViewReconnecting() {
  return (
    <div className="game-menu">
      <h2 className="game-menu__title">
        Esperando a que el jugador se reconecte...
      </h2>
    </div>
  );
}
...
function GameViewContent({
  ...
  const { gameState, leaveGame, setGameType, isMyTurn, isPlayer, isViewer } =
    useGame();
    ...
    <div ref={containerRef} className="game-view">
      ...
      >
        ...
        {gameState.status === 'PLAY' &&
          !isMyTurn &&
          isPlayer &&
          gameState.mode === 'online' && <GameViewWait />}
