...
export const Profile = () => {
   ...
   */
  const postsOwnerIDRef = useRef<number | null>(null);
   ...
   */
  useEffect(() => {
    const ownerID = profileUser?.id ?? null;
    ...
    const loadInitialPosts = async () => {
      try {
        ...
        if (!cancelled && postsOwnerIDRef.current === ownerID) {
          setPostsError("The posts could not be loaded.");
        }
      } finally {
        if (!cancelled && postsOwnerIDRef.current === ownerID) {
          setIsLoadingPosts(false);
  ...
  useEffect(() => {
    ...
    setProfileUser((currentProfile) => {
      ...
      return {
        ...currentProfile,
        name: authenticatedUser.name ?? currentProfile.name,
        surname: authenticatedUser.surname ?? currentProfile.surname,
        avatarPath: authenticatedUser.avatarPath ?? null,
        bannerPath: authenticatedUser.bannerPath ?? null,
      };
    });
  }, [
  ...
  if (!authenticatedUser) {
    return (
      <div className="error-screen">No se ha podido cargar tu sesión.</div>
    );
  }
  ...
  if (profileError || !profileUser) {
    return (
      <div className="error-screen">
        {profileError ?? "The profile could not be loaded."}
      </div>
    );
  }
    ...
    <div className="profile">
      <div className="profile__container">
        <ProfileBanner
          ...
          onEdit={() => setIsBannerEditorOpen(true)}
          onImageError={() => setBannerImageFailed(true)}
        />
        ...
        <ProfileHeader
          ...
          canSendRequest={profileUser.can_send_request}
        ...
        >
          <div className="profile__posts">
            {isLoadingPosts && (
              <div className="profile__posts-state">Loading posts.</div>
            )}

            {!isLoadingPosts && profilePosts.length === 0 && postsError && (
              <div className="profile__posts-error">{postsError}</div>
            )}
      ...
      {currentConfirmation && (
        <ConfirmModal
          ...
          confirmingLabel={currentConfirmation.confirmingLabel}
          cancelLabel="Cancelar"
          isConfirming={isConfirming}
          onConfirm={handleConfirmRelationAction}
